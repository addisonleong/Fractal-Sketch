    ...
    for (int i = 0; i != step + 1; i++) {
        if (g1 == 1) {
            for (int a = p; a != p + pow(gens, i) * vertices; a++) {
                pt = ApplyMatrixToPoint([[xs objectAtIndex:(a)]intValue], [[ys objectAtIndex:(a)]intValue], r1);
                pt = ApplyMatrixToPoint(pt.c[0][0], pt.c[1][0], s1);
                pt = ApplyMatrixToPoint(pt.c[0][0], pt.c[1][0], MultiplyMatrices(t1, GenerateTranslationMatrix((-1 * (cx)), (-1 * (cy)))));
        ...
        if (g2 == 1) {
            for (int a = p; a != p + pow(gens, i) * vertices; a++) {
                pt = ApplyMatrixToPoint([[xs objectAtIndex:(a)]intValue], [[ys objectAtIndex:(a)]intValue], r2);
                pt = ApplyMatrixToPoint(pt.c[0][0], pt.c[1][0], s2);
                pt = ApplyMatrixToPoint(pt.c[0][0], pt.c[1][0], MultiplyMatrices(t2, GenerateTranslationMatrix((-1 * (cx)), (-1 * (cy)))));
                NSNumber *xy = [NSNumber numberWithDouble:(pt.c[0][0])];
                NSNumber *yx = [NSNumber numberWithDouble:(pt.c[1][0])];
                [xs addObject:xy];
                [ys addObject:yx];
        ...
        if (g3 == 1) {
            for (int a = p; a != p + pow(gens, i) * vertices; a++) {
                pt = ApplyMatrixToPoint([[xs objectAtIndex:(a)]intValue], [[ys objectAtIndex:(a)]intValue], r3);
                pt = ApplyMatrixToPoint(pt.c[0][0], pt.c[1][0], s3);
                pt = ApplyMatrixToPoint(pt.c[0][0], pt.c[1][0], MultiplyMatrices(t3, GenerateTranslationMatrix((-1 * (cx)), (-1 * (cy)))));
                NSNumber *xy = [NSNumber numberWithDouble:(pt.c[0][0])];
                NSNumber *yx = [NSNumber numberWithDouble:(pt.c[1][0])];
                [xs addObject:xy];
                [ys addObject:yx];
        ...
        if (g4 == 1) {
            for (int a = p; a != p + pow(gens, i) * vertices; a++) {
                pt = ApplyMatrixToPoint([[xs objectAtIndex:(a)]intValue], [[ys objectAtIndex:(a)]intValue], r4);
                pt = ApplyMatrixToPoint(pt.c[0][0], pt.c[1][0], s4);
                pt = ApplyMatrixToPoint(pt.c[0][0], pt.c[1][0], MultiplyMatrices(t4, GenerateTranslationMatrix((-1 * (cx)), (-1 * (cy)))));
                NSNumber *xy = [NSNumber numberWithDouble:(pt.c[0][0])];
                NSNumber *yx = [NSNumber numberWithDouble:(pt.c[1][0])];
                [xs addObject:xy];
                [ys addObject:yx];
...
void guide(CGContextRef context) {
    
}
void grids(CGContextRef context) {
    if ([[UIDevice currentDevice] orientation] == UIDeviceOrientationPortrait) {
    for (float x = 768%30; x <= 1536; x = x + 30) {
        CGContextSetLineWidth(context, 1);
        CGContextSetStrokeColorWithColor(context, [[UIColor grayColor]CGColor]);
        CGContextMoveToPoint(context, x, 0);
        CGContextAddLineToPoint(context, x, 2048);
        CGContextSaveGState(context);
        CGContextStrokePath(context);
    }
    for (float y = 1024%30; y <= 2048; y = y + 30) {
        CGContextSetLineWidth(context, 1);
        CGContextSetStrokeColorWithColor(context, [[UIColor grayColor]CGColor]);
        CGContextMoveToPoint(context, 0, y);
        CGContextAddLineToPoint(context, 1536, y);
        CGContextSaveGState(context);
        CGContextStrokePath(context);
    }
    }
    else {
        for (float x = 1024%30; x <= 2048; x = x + 30) {
            CGContextSetLineWidth(context, 1);
            CGContextSetStrokeColorWithColor(context, [[UIColor grayColor]CGColor]);
            CGContextMoveToPoint(context, x, 0);
            CGContextAddLineToPoint(context, x, 1536);
            CGContextSaveGState(context);
            CGContextStrokePath(context);
        }
        for (float y = 768%30; y <= 1536; y = y + 30) {
            CGContextSetLineWidth(context, 1);
            CGContextSetStrokeColorWithColor(context, [[UIColor grayColor]CGColor]);
            CGContextMoveToPoint(context, 0, y);
            CGContextAddLineToPoint(context, 2048, y);
            CGContextSaveGState(context);
            CGContextStrokePath(context);
        }
    }
...
void drawGuides(CGContextRef context) {
    CGContextSetLineWidth(context, 1);
